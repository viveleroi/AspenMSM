...
*/
				...
				if (opts.showCounts){
					...
					$(window).load(function(){
						var count = getLetterCount('a.ln-selected');
...
/*						var width = ($(this).outerWidth({margin:true})-1) + 'px'; // the -1 is to tweak the width a bit due to a seeming inaccuracy in jquery ui/dimensions outerWidth (same result in FF2 and IE6/7) */
						...
						$letterCount.text( count ); // set left position and width of letter count, set count text and show it
					});
 ...
 */
			...
			function createLettersHtml(){
				...
				return '<div class="ln-letters clearfix">' + html.join('') + ((opts.showCounts) ? '<div class="ln-letter-count">0</div>' : '' + '</div>'); // the styling for ln-letter-count is to give us a starting point for the element, which will be repositioned when made visible (ie, should not need to be styled by the user)
			}				
			init();
		});
	};
