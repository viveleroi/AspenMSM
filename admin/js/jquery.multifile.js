    ...
    return $(this).each(function(group_count){
     if(this._MultiFile) return; this._MultiFile = true;
       ...
       MF.addSlave = function( slave, slave_count ){
        ...
        $(slave).change(function(){
          ...
          if(ERROR!=''){
            ...
            // 2007-06-24: BUG FIX - Thanks to Adrian WrÛbel <adrian [dot] wrobel [at] gmail.com>
       ...
       MF.addToList = function( slave ){
        ...
        var
         r = $('<div class="img-list-item"></div>'),
         v = String(slave.value || ''/*.attr('value)*/),
         a = $('<span class="file" title="'+MF.STRING.selected.replace('$file', v)+'">'+MF.STRING.file.replace('$file', v.match(/[^\/\\]+$/gi)[0])+'</span>'),
         b = $('<a href="#'+MF.wrapID+'" class="remove" title="Are you sure you want to remove this image?">'+MF.STRING.remove+'</a>');
        
        // Insert label
        MF.list.append(
         r.append(' ', a, b)//.prepend(slave.i+': ')
        );
