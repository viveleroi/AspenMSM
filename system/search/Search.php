...
class Search {
	...
	public function load_content(){
		...
		$this->APP->model->query('TRUNCATE search_content_index');
			...
			foreach($pages as $page){
				...
				if(is_array($contents) && !empty($contents)){
					foreach($contents as $content){
						...
						$to_index = $this->APP->director->loadIndexer($content['type'], $content);

						// for the content, store in the search index table
						if(is_array($to_index)){
							foreach($to_index as $content_chunk){
								$this->APP->model->executeInsert('search_content_index', $content_chunk);
							}
						}
					}
				}
	...
	public function search($add_params = false){
		...
		if($type){
			$results = $this->APP->director->moduleSearch($type, $keyword, $add_params);
		}

		// if no results, run normal
		if(!$results && $keyword){
			$this->APP->model->enablePagination();
			$this->APP->model->select('search_content_index');
