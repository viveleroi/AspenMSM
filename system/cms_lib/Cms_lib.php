...
class Cms_lib {
	...
	private function knowAllLivePages(){
		...
		$this->APP->model->orderBy('page_sort_order');
	...
	private function knowAllPrivatePages(){
		$this->APP->model->select('pages');
		$this->APP->model->where('page_is_live', 0);
		$this->APP->model->orderBy('page_sort_order');
		$pages = $this->APP->model->results();
	...
	private function identifyPage(){
		...
		if(isset($uri[0]) && !empty($uri[0])){
			...
			$final_page = array();
			$final_page['page'] = $this->APP->model->quickSelectSingle('pages', $this->APP->settings->getConfig('home_page'), 'page_id');
		...
		if(isset($final_page['page']['page_id'])) {
			$this->APP->model->select('pages');
			$this->APP->model->where('parent_id', $final_page['page']['page_id']);
			$this->APP->model->where('is_parent_default', 1);
			$pages = $this->APP->model->results();
	...
	public function content($page = false){
		...
		if($page){
			...
			$sections = $this->APP->model->query($sql);
			if($sections->RecordCount()){
				while($section = $sections->FetchRow()){
					
					$section_data = $this->APP->director->readPageSections($section);
	...
	public function getContent($placement_group = false){
		...
		if(count($content) == 0){
			...
			if(!empty($placement_group) && isset($this->page['page_template'])){
				...
				$this->APP->model->where('template', $this->page['page_template']);
				$this->APP->model->where('group_name', $placement_group);
				$exists = $this->APP->model->results();
				
				if(!$exists['RECORDS']){
					$this->APP->model->query(sprintf('INSERT INTO template_placement_group (template, group_name) VALUES ("%s", "%s")',
	...
	public function display_content($section = false){
		...
		if($content){
			foreach($content as $section){
				$this->APP->director->displayPageSections($section, $this->page, $this->bits);
			}
			return true;
		} else {
			return false;
