...
class Director {
	...
	public function loadPageSection($type = false, $next_id = false, $section_data = false, $page_id = false, $template = false){
		...
		if($load_sec){
			if(isset($this->APP->{$load_sec['module']}) && method_exists($this->APP->{$load_sec['module']}, 'sectionEditor')){
				$this->APP->{$load_sec['module']}->sectionEditor($type, $next_id, $section_data, $page_id, $template);
			}
		}
	}
		...
		if(isset($post['page_sections']) && is_array($post['page_sections'])){
			...
			foreach($post['page_sections'] as $section){
				if(array_key_exists($section['section_type'], $this->sections)){
					$this->APP->model->query(sprintf('DELETE FROM section_%s WHERE page_id = "%s"', $section['section_type'], $id));
				}
			}
			...
			foreach($post['page_sections'] as $val){
				if(array_key_exists($val['section_type'], $this->sections)){
					...
					if(isset($this->APP->{$section['module']}) && method_exists($this->APP->{$section['module']}, 'saveSection')){
						$new_sections = $this->APP->{$section['module']}->saveSection($val, $id);
	...
	public function readPageSections($section_data){
		...
		foreach($this->sections as $section){
			if($section_data['section_type'] == $section['type']){
				if(isset($this->APP->{$section['module']}) && method_exists($this->APP->{$section['module']}, 'readSection')){
					$new_sections = $this->APP->{$section['module']}->readSection($section_data);
					$sections = array_merge($sections, $new_sections);
				}
			}
		}
	...
	public function displayPageSections($content, $page, $bits){
		...
		if(!$this->APP->cms_lib->error()){
		
			$sections = array();
			
			foreach($this->sections as $section){
				if($content['type'] == $section['type']){
					if(isset($this->APP->{$section['module']}) && method_exists($this->APP->{$section['module']}, 'displaySection')){
						$this->APP->{$section['module']}->displaySection($content, $page, $bits);
					}
				}
			}
	...
	public function loadIndexer($type = false, $content = false){
		...
		// if we have a matched module, load the proper function
		if($load_sec){
			if(isset($this->APP->{$load_sec['module']}) && method_exists($this->APP->{$load_sec['module']}, 'searchIndexer')){
				$output = $this->APP->{$load_sec['module']}->searchIndexer($content);
			}
		}
	...
	public function moduleSearch($type = false, $keyword = false, $add_params = false){
		...
		// if we have a matched module, load the proper function
		if($load_sec){
			if(isset($this->APP->{$load_sec['module']}) && method_exists($this->APP->{$load_sec['module']}, 'search')){
				$results = $this->APP->{$load_sec['module']}->search($keyword, $add_params);
			}
		}
